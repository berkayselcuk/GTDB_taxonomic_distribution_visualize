/* lineage.js — v3.1 (fixed filter composition + normalization order + “Differences” panel)
 * =======================================================================
...
(function (global) {
  ...
  let totalInput = 0, geneNames = [], matrix = null, ASM_COUNT = 0;
  const countMap = new Map(), asmIndex = new Map(), geneIndex = new Map();
  const coordMap = new Map(), widthMap = new Map();
  let normalizeLevel = null, xBand = null;
  ...
  const sizeLevelSel       = d3.select('#sizeFilterLevel');
  const sizeThresholdInput = d3.select('#sizeFilterThreshold');
  const sizeFilterBtn      = d3.select('#applySizeFilter');
  const resetSizeFilterBtn = d3.select('#resetSizeFilter');

  /* ────────────────────────────────────────────────────────
     PRESENCE TOGGLE
     ------------------------------------------------------- */
  function togglePresence() {
    for(let i=0; i<matrix.length; ++i) matrix[i] = matrix[i] ? 0 : 1;
    updateRugs();
    togglePresenceBtn.text(
      togglePresenceBtn.text()==='Show Absence'
  ...
  function getColorScale(lv,cats) {
    if(!COLOR_CACHE[lv]) {
      COLOR_CACHE[lv] = d3.scaleOrdinal(
        cats, cats.map((_,i)=>d3.interpolateRainbow((i*GOLDEN)%1))
      );
    }
  ...
  function pickTSV(cb) {
    ...
    ip.addEventListener('change',e=>{
      ...
      const r = new FileReader(); r.onload=ev=>cb(ev.target.result);
      r.readAsText(f);
    });
  ...
  function buildLayout() {
    ...
    if(!normalizeLevel) {
      ...
      assemblies.forEach(a=>{ coordMap.set(a,xBand(a)); widthMap.set(a,xBand.bandwidth()); });
      return;
    }
    if(normalizeLevel==='__ALL__') {
      const w = totalW/assemblies.length;
      assemblies.forEach((a,i)=>{ coordMap.set(a,i*w); widthMap.set(a,w); });
      return;
    }
    const runs=[],lvl=normalizeLevel;
    let start=0, cat=raw[0][lvl];
    for(let k=1;k<assemblies.length;++k) {
      if(raw[k][lvl]!==cat) { runs.push({cat,start,end:k-1}); cat=raw[k][lvl]; start=k; }
    }
    ...
    runs.forEach((run,ri)=>{
      ...
      arr.forEach((a,idx)=>{ coordMap.set(a,ri*segW+idx*w); widthMap.set(a,w); });
    });
  }
  ...
  function updateMappingBanner() {
    ...
    const pctInput = totalInput ? ((matched/totalInput)*100).toFixed(1) : '0.0';
    const pctGTDB  = ASM_COUNT ? ((matched/ASM_COUNT)*100).toFixed(1) : '0.0';
    mapInfo.html(
  ...
  // ────────────────────────────────────────────────────────
  // initSizeFilterControls()
  // ────────────────────────────────────────────────────────
  function initSizeFilterControls() {
    sizeFilterBtn.on('click', applyLineageSizeFilter);
    resetSizeFilterBtn.on('click', () => {
      resetFilter();
      sizeLevelSel.property('value','');
      sizeThresholdInput.property('value',0);
    });
  }

  function applyLineageSizeFilter() {
    const level = sizeLevelSel.property('value');
    const threshold = parseInt(sizeThresholdInput.property('value'),10);
    if(!level || isNaN(threshold) || threshold<0) {
      return alert('Choose a lineage level and non-negative threshold');
    }
    const counts = d3.rollup(raw, v=>v.length, d=>d[level]);
    raw = raw.filter(d=>counts.get(d[level])>=threshold);
    updateView();
  }
  ...
  function hookCheckboxContainer(sel) {
    sel.on('change',e=>{
      if(e.target.matches("input[type='checkbox']")) onGeneCheckboxChange.call(e.target);
    });
  }
  ...
  function buildSearchDatalist() {
    const cats = new Set(); originalRaw.forEach(d=>allLevels.forEach(lv=>cats.add(d[lv])));
    optList.html(''); optList.selectAll('option')
      .data(Array.from(cats).sort()).enter().append('option').attr('value',d=>d);
  }
  ...
  /* ────────────────────────────────────────────────────────
     ADD DIFFERENCE ROWS
     ------------------------------------------------------- */
  function addDiffRows(){ /* unchanged from original */
    const g1=diff1Sel.property('value'), g2=diff2Sel.property('value'), useC=useCnt.property('checked');
    ...
    const newM=new Uint8Array(geneNames.length*ASM_COUNT); newM.set(matrix);
    assemblies.forEach(a=>{
      const ai=asmIndex.get(a), cm=countMap.get(a)||{};
      ...
      cm[name1]=p1; cm[name2]=p2; countMap.set(a,cm);
    });
    matrix=newM;
    [name1,name2].forEach(lbl=>{
      diffSelCt.append('label').text(lbl)
        .append('input').attr('type','checkbox').attr('value',lbl).property('checked',true);
      activeGenes.push(lbl);
      drawRugRow(lbl,activeGenes.length-1,selectedLevels.length*LEVEL_HEIGHT+BASE_GAP);
    });
  ...
  function initNormaliseControls(){
    normSel.html(''); normSel.append('option').attr('value','').text('— choose level —');
    allLevels.forEach(lv=>normSel.append('option').attr('value',lv).text(lv));
    normBtn.on('click',()=>{
      const lv=normSel.property('value'); normalizeLevel=lv||null;
      updateView();
    });
  ...
  /* ────────────────────────────────────────────────────────
     DRAW LINEAGE BLOCKS
     ------------------------------------------------------- */
  function drawLineage(){ /* unchanged from original */
    plot.selectAll('.level').remove(); if(!raw.length) return;
    const counts={}; selectedLevels.forEach(lv=>counts[lv]=d3.rollup(raw,v=>v.length,d=>d[lv]));
    selectedLevels.forEach((lv,i)=>{
      ...
      g.selectAll('rect').data(runs).join('rect')
        ...
        .attr('y',0)
        .attr('width',d=>getX(assemblies[d.end])+getW(assemblies[d.end])-getX(assemblies[d.start]))
  ...
  /* ────────────────────────────────────────────────────────
     ...
     ------------------------------------------------------- */
  const activeGenes=[];
  function drawRugRow(gene,idx,baseY){ /* unchanged */
    const y=baseY+idx*(RUG_HEIGHT+RUG_PAD);
    const gi=geneIndex.get(gene); if(gi===undefined) return;
    assemblies.forEach(a=>{ const ai=asmIndex.get(a);
      if(matrix[gi*ASM_COUNT+ai]) rugGrp.append('rect')
        .attr('data-gene',gene).attr('x',getX(a)).attr('y',y)
        .attr('width',getW(a)).attr('height',RUG_HEIGHT);
    });
    rugLbl.append('text')
      .attr('data-gene',gene).attr('x',-10).attr('y',y+RUG_HEIGHT/2)
      .attr('dy','.35em').attr('text-anchor','end')
      .text(gene.replace(/_count$/,''));
  }
  function updateRugs(){ rugGrp.selectAll('*').remove(); rugLbl.selectAll('*').remove();
    const baseY=selectedLevels.length*LEVEL_HEIGHT+BASE_GAP;
    activeGenes.forEach((g,i)=>drawRugRow(g,i,baseY)); setSVGSize(); }
  function removeRugRow(g){ rugGrp.selectAll(`rect[data-gene=\"${g}\"]`).remove(); rugLbl.selectAll(`text[data-gene=\"${g}\"]`).remove(); }
  function shiftRugRow(g,newIdx){ const y=selectedLevels.length*LEVEL_HEIGHT+BASE_GAP+newIdx*(RUG_HEIGHT+RUG_PAD);
    rugGrp.selectAll(`rect[data-gene=\"${g}\"]`).attr('y',y);
    rugLbl.selectAll(`text[data-gene=\"${g}\"]`).attr('y',y+RUG_HEIGHT/2);
  }

  function onGeneCheckboxChange(){ /* unchanged */
    const gene=this.value; if(this.checked){
      if(!activeGenes.includes(gene)){
        activeGenes.push(gene);
        drawRugRow(gene,activeGenes.length-1,selectedLevels.length*LEVEL_HEIGHT+BASE_GAP);
      }
    } else {
      const i=activeGenes.indexOf(gene);
      if(i>-1){ activeGenes.splice(i,1); removeRugRow(gene); activeGenes.forEach((g,j)=>shiftRugRow(g,j)); }
    }
    setSVGSize(); toggleBtn.text(activeGenes.length?'Deselect All':'Select All');
  }

  function toggleAll(){ /* unchanged */
    const boxes=d3.selectAll('#geneSelector input[type="checkbox"], #diffSelector input[type="checkbox"]');
    if(activeGenes.length){ boxes.property('checked',false); activeGenes.slice().forEach(removeRugRow); activeGenes.length=0; }
    else { boxes.property('checked',true); boxes.nodes().forEach(n=>{ const g=n.value; if(!activeGenes.includes(g)){ activeGenes.push(g); drawRugRow(g,activeGenes.length-1,selectedLevels.length*LEVEL_HEIGHT+BASE_GAP); } }); }
    setSVGSize(); toggleBtn.text(activeGenes.length?'Deselect All':'Select All');
  }
    ...
    pickTSV(tsvText=>{
      Promise.resolve()
        ...
        .then(jsonData=>{
          ...
          assemblies=raw.map(d=>d.assembly); ASM_COUNT=assemblies.length;
          assemblies.forEach((a,i)=>asmIndex.set(a,i));
          const lines=tsvText.trim().split(/\r?\n/);
          const header=lines.shift().split('\t');
          const rows=lines.filter(Boolean); totalInput=rows.length;
          geneNames=header.filter(h=>h.endsWith('_count'));
          geneNames.forEach((g,i)=>geneIndex.set(g,i));
          matrix=new Uint8Array(geneNames.length*ASM_COUNT); countMap.clear();
          d3.tsvParseRows(rows.join('\n'),row=>{
            const asm=row[0]; if(!asmIndex.has(asm)) return;
            const ai=asmIndex.get(asm), cm={};
            geneNames.forEach((g,gi)=>{ const cnt=+row[header.indexOf(g)]||0; cm[g]=cnt; if(cnt>0) matrix[gi*ASM_COUNT+ai]=1; });
            countMap.set(asm,cm);
          });
          buildLevelSelector(); buildGeneSelector(); buildSearchDatalist(); bindSearch();
          populateDiffSelectors(); initNormaliseControls();
  ...
  global.LineageVis = {
    ...
    filter: ()=>drawLineage(),
    normalise: lvl=>{ normalizeLevel=lvl||null; updateView(); }
  };
